...
function telaCoisa(coisaId,volta,eco){
  ...
  /* ---- olhar: de graça, e insinua ---- */
  if(c.verbos.includes('olhar'))botao(COISA_CFG.verbos.olhar.n,()=>{
    e.visto++;
    const t=acharFoco(coisaId,false);
    telaCoisa(coisaId,volta,[t?[t.txt,'alerta']:['É o que parece ser.','fraco']]);
  },{custo:COISA_CFG.verbos.olhar.d});

  /* ---- escutar: silencioso, e ENTREGA o foco ---- */
  if(c.verbos.includes('escutar'))botao(COISA_CFG.verbos.escutar.n,()=>{
    e.visto++;
