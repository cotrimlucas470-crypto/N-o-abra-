...
/* ---------- checagem de realidade ----------
   8 minutos e 4 de estresse, e acerta 80% das vezes. Os 20% restantes
   não devolvem "não sei": devolvem a resposta errada com a mesma cara de
   certeza. Uma checagem que avisasse quando falhou seria de 100%. */
function checarRealidade(){
  const acertou=chance(.8);
  const eFalso=!!S.ilusaoNoAr;
  const resposta=acertou?eFalso:!eFalso;
  v9Perder(4,'CHECAGEM');
  S.hora=Math.min(19,(S.hora||7));
